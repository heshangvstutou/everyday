...
public class TetherSettingsExt implements OnPreferenceClickListener,
    ...
    private void initPreference(PreferenceScreen screen) {
        ...
        // TetherSettings.java
	Log.d(TAG,"jlink zhangjie  usbRegexs.length======"+usbRegexs.length+"mUsbTetherType=="+mUsbTetherType);
        boolean usbAvailable = usbRegexs.length != 0;
        if (usbAvailable || Utils.isMonkeyRunning()) {
            if (mUsbTetherType != null) {
                Log.d(TAG, "jlink remove mUsbTetherType");
    ...
    public void updateUSBPrfSummary(Preference pref, String originSummary ,
        boolean usbTethered, boolean usbAvailable) {
        if (usbTethered) {
            if (FeatureOption.MTK_TETHERINGIPV6_SUPPORT) {
                pref.setSummary(originSummary
