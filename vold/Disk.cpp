...
void Disk::createPrivateVolume(dev_t device, const std::string& partGuid) {
    ...
    if (major(mDevice) == android::vold::Disk::kMajorBlockMmc) {
        // only the external sdcard can format as internal
        if (minor(mDevice) != kMinorBlockEMMC) {
            vol->setStorageType(android::vold::VolumeBase::StorageType::kExternalSD);
        }
    }
...
if (major(mDevice) == kMajorBlockLoop && minor(mDevice) == kMinorBlockFon) {
    LOG(WARNING) << "FAT on NAND loop device, skip sgdisk";
    ...
    for (auto line : output) {
        ...
        if (!strcmp(token, "DISK")) {
            ...
            if (table == Table::kMbr) {
                ...
                const char* partDescription = strtok(nullptr, kSgdiskToken);

                /* For emmc, only handle 'intsd' partition */
                if ( major(mDevice) == kMajorBlockMmc && minor(mDevice) == kMinorBlockEMMC && strcasecmp(partDescription, "intsd") ) {
                     LOG(WARNING) << mId << " is ignoring partition " << i
                                      << ". For emmc, it only needs to handle 'internal sd' partition";
                     continue;
                }
                ...
                else if (!strcasecmp(typeGuid, kGptAndroidMeta)) {
                    if (mJustPartitioned) {
                        char sector0[512];
                        memset(sector0, 0x00, sizeof(sector0));
                        std::string devPath = StringPrintf("/dev/block/vold/%u,%u",  major(partDevice), minor(partDevice));
                        CreateDeviceNode(devPath, partDevice);

                        if (WipeBlockDevice(devPath) != OK) {
                            LOG(WARNING) << getId() << " failed to wipe";
                        }

                        int fd = open(devPath.c_str(), O_RDWR | O_CLOEXEC);
                        if (fd < 0) {
                            PLOG(ERROR) << getId() << " failed to open " << devPath;
                        }
                        else {
                            if (write(fd, sector0, sizeof(sector0)) != sizeof(sector0)) {
                               PLOG(ERROR) << getId() << " failed to write sector0";
                            }
                            close(fd);
                        }
                        DestroyDeviceNode(devPath);
                    }
                }
            }
        }
    ...
    if (major(mDevice) == kMajorBlockMmc && minor(mDevice) == kMinorBlockEMMC && table == Table::kMbr) {
        if (!access("/emmc@fat", F_OK)) {
            LOG(DEBUG) << "/emmc@fat exists";

            char link[PATH_MAX];
            int length = readlink("/emmc@fat", link, sizeof(link)- 1);
            if (length > 0) {
                link[length] = 0;
                int index = strlen(link)-1;
                while(isdigit(link[index])) {
                  index--;
                }
                int partDeviceMinor = strtol(&link[index+1], nullptr, 10);
                LOG(DEBUG) << "/emmc@fat minor is " << partDeviceMinor << ", try to new it as PublicVolume";
                dev_t partDevice = makedev(major(mDevice), minor(mDevice) + partDeviceMinor);
                createPublicVolume(partDevice);
            }
        }
    }
...
status_t Disk::partitionPublic() {
    ...
    /* For emmc, don't re-create MBR, and only format 'intsd' partition */
    if (major(mDevice) == kMajorBlockMmc && minor(mDevice) == kMinorBlockEMMC) {
       LOG(WARNING) << "For emmc, don't re-create MBR and only format 'internal sd' partition";
       readMetadata();
       readPartitions();
       return 0;
    }
